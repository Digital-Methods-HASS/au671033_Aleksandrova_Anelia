...
ggplot(data = Western_Europe, aes(x = year, y = homicides_per_100k, color = entity)) + 
  geom_smooth() +
  labs(title = "Homicide rates", x = "Year", y = "Homicides per 100,000")
```
...
When comparing the two trends we can see that the homicide rates are going down, while the reign duration is becoming steady and even starting to increase. 

```{r eval = TRUE}
danish_kings <- read.csv('data/danish_kings.csv', sep = ";") 
danish_kings <- na.omit(danish_kings)
danish_kings$midyear <- danish_kings$reign_end_year -(danish_kings$reign_end_year -danish_kings$reign_start_year)/2

ggplot(data = danish_kings, aes(x = midyear, y = reign_duration)) +
  geom_smooth() +
  labs(title = "Duration among Danish rulers through time", x = "Midyear", y = "Reign duration") 
...
Based in the visualizations I would say that we are more civilized, because the homicide rates are decreasing. Crime and homicide rates are usually affected by factors such as poverty, economic inequality, ethnic fractionalization etc., and less homicide means a decrease in those aspects as well. 
